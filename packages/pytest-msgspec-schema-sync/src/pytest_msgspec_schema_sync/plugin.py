...
def pytest_addoption(parser):
    ...
    parser.addini(
        ...
        default=str(msgspec.structs.asdict(PluginConfig())),
    )
...
class PSSItem(pytest.Item):
    def __init__(self, name: str, parent: pytest.Collector, field: Info):
        ...
        self.field = field
        global pss_ini
        plugin_config = msgspec.convert(pss_ini, PluginConfig)
        self.plugin_config = plugin_config

    def get_root_dir(self, root: Literal["package_root", "repo_root"]) -> Path:
        assert root in (opts := ["package_root", "repo_root"]), f"{root=} not in {opts}"
        match root:
            case "package_root":
                hint = "pyproject.toml"
            case "repo_root":
                hint = ".git"
        try:
            dir_path = next(d for d in self.path.parents if any(d.glob(hint)))
